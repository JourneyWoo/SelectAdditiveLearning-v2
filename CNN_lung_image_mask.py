...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  2 00:24:03 2017
...
from lung_image_mask import read_data_maskAndimage
...
def cnn_model():
    ...
    X_train, X_test, y_train, y_test = read_data_maskAndimage()
    ...
    y = tf.placeholder(tf.float32, shape = [None, image_size, image_size])
    ...
    W_fc2 = weight_variable([hidden, image_size * image_size])
    b_fc2 = bias_variable([image_size * image_size])
    
    y_conv = tf.nn.softmax(tf.matmul(h_fc1, W_fc2) + b_fc2)
    y_conv = tf.reshape(y_conv, [batch_size, 128, 128])
    ...
    with tf.Session() as sess:
        ...
        count = 1
        itr = X_train.shape[0] / batch_size        #240
        
        for num_1 in range(epochs):
            i = 0
            for num_2 in range(itr):
                X_train_batch = X_train[i : batch_size * (num_2 + 1)]
                X_train_batch = X_train_batch.reshape([batch_size, 128, 128])
            
                y_train_batch = y_train[i : batch_size * (num_2 + 1)]
                y_train_batch = y_train_batch.reshape([batch_size, 128, 128])
                
                i = i + batch_size
                
                
                feed_dict_1 = {x: X_train_batch, y: y_train_batch}
                feed_dict_2 = {x: X_train_batch, y: y_train_batch}
                
                sess.run(optimize, feed_dict = feed_dict_1)
                loss, acc = sess.run([cross_entropy, accuracy], feed_dict = feed_dict_2)
