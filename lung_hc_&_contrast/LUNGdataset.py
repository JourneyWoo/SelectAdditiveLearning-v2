...
import numpy as np
import dicom as dm
from LUNGpreprocess import load, get_pixels_hu, resample, normalize, zero_center, folder_travel
import scipy
...
def make_dataset():
    ...
    path_cancer = '/Users/wuzhenglin/Python_nice/SAL_LUNG/test_data/c'
    slices_cancer = load(path_cancer)
    get_pixels_hu_cancer = get_pixels_hu(slices_cancer)
    
    normalize_cancer = normalize(get_pixels_hu_cancer)
    ...
    path_healthy = '/Users/wuzhenglin/Python_nice/SAL_LUNG/test_data/h'

    slices_healthy = load(path_healthy)
    get_pixels_hu_healthy = get_pixels_hu(slices_healthy)
    normalize_healthy = normalize(get_pixels_hu_healthy)
    ...
    writer= tf.python_io.TFRecordWriter("new_test_hc_128.tfrecords") 
