...
def findcontrast(dcm_name, overview_df):
    
    
    a = overview_df[overview_df['dicom_name'] == dcm_name].head()['idx']
    b = np.asarray(a)[0]
    c = overview_df[overview_df['idx'] == b].head()['Contrast']
    d = np.asarray(c)[0]
...
def make_dataset():
    ...
    writer= tf.python_io.TFRecordWriter("contrast_lung_dataset_test.tfrecords") 
    ...
    for i in range(20, 60, 2):
        ...
        dcm_name = lis[i]
        lab = findcontrast(dcm_name, overview_df)
