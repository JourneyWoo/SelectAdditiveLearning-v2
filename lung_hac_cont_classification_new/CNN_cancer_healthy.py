...
def cnn_model():
    ...
    epochs = 4    #42
    batch_size = 380     #20
    learning_rate = 0.00006       #0.05
    hidden = 4
    cap_c = 1214
    cap_h = 1290
    num = cap_c + cap_h #2505
    image_size = 128
    label_size = 2
...
#    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels = y, logits = out_feed))
...
#    correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(y, 1))
    ...
    cwd = '/Users/wuzhenglin/Python_nice/SAL_LUNG/lung_hc_contrast_classification/new_train_hc_128_2.tfrecords'
    im, lab = read_and_decode(cwd)
    img_batch, label_batch = tf.train.shuffle_batch([im, lab],
                                            batch_size = batch_size, capacity = num,
                                            min_after_dequeue = 55)

    cwd_ = '/Users/wuzhenglin/Python_nice/SAL_LUNG/lung_hc_contrast_classification/new_test_hc_128.tfrecords'
    im_, lab_ = read_and_decode(cwd_)  
    img_batch_, label_batch_ = tf.train.shuffle_batch([im_, lab_],
                                            batch_size = 20, capacity = 20,
                                            min_after_dequeue = 5)
